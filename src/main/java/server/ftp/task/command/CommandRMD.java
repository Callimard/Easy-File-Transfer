...
public class CommandRMD extends FTPCommand {

	// Constants.

	public static final String NAME_CODE = "RMD";

	// Constructors.

	public CommandRMD(FTPClientConnection clientConnection, FTPServerController ftpServerController, String... args) {
			...
			if (ftpAuthenticationManager.isAlreadyAuthenticated(this.getClientConnection())) {
				...
				if (args != null && args.length >= 1) {
					...
					try {
						...
						ftpFileManager.deleteDirectory(workingPath, this.getClientConnection());

						report.offerAction("Suppression du dossier.");

						this.getClientConnection().write("250 directory deleted.\n");

					} catch (IOException e) {

						report.offerAction("Erreur de fichier = " + e);
						report.setFailed();

						this.getClientConnection().write("550 path error\n");
					} catch (PathIsNotDirectoryException e) {

						report.offerAction("Erreur de fichier = " + e + ". Le path n'est pas un repertoire.");
						report.setFailed();

						this.getClientConnection().write("550 path is not a directory\n");
					} catch (NoAccessToThisFileException e) {

						report.offerAction("Access refuser pour FileToTransfert = " + workingPath);
						...
						report.offerAction("Access refuser pour Path = " + workingPath);
						report.offerAction("Error = " + e);
						report.setFailed();
