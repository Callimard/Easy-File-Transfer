...
public class CommandSTOR extends FTPCommand implements FinishListener {

	// Constants.

	public static final String NAME_CODE = "STOR";
	...
	public CommandSTOR(FTPClientConnection clientConnection, FTPServerController ftpServerController, String... args) {
				...
				if (args != null && args.length >= 1) {
					...
					try {
						...
						try {
							...
							try {
								if (ftpFileManager.writeAccess(localPath.getParent(), ftpClientConnection)) {
									...
									report.offerAction("Access refuser pour ParentPath = " + localPath.getParent());
									report.offerAction("Error = " + localPath.getParent());
									report.setFailed();
								...
								report.offerAction(
										"Le path n'est pas un path dans la zone de fichier du serveur. Path = "
												+ localPath.getParent());
						...
						report.offerAction("Probl�me de format du path. Erreur = " + e);
						report.setFailed();
